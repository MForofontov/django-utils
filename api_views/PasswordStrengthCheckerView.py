...
class PasswordStrengthCheckerView(APIView):
    ...
    def post(self, request):
        ...
        if len(password) >= 8 and re.search(r"[A-Za-z]", password) and re.search(r"\d", password):
            strength = "medium"
        if len(password) >= 12 and re.search(r"[A-Za-z]", password) and re.search(r"\d", password) and re.search(r"[!@#$%^&*()_+]", password):
            strength = "strong"

        # Return the password strength as a JSON response
        return Response({"password_strength": strength}, status=status.HTTP_200_OK)
